...
def update_github_repositories(main_directory, include_aur):
    ...
    if not found_repos:
        print("\nNo GitHub repositories found in the current directory or its subdirectories. Exiting.")
        exit()
    else:
...
def main():
    ...
    if not any('.git' in root for root, _, _ in os.walk(current_directory)):
        print("\nYou need to be inside a directory with GitHub repositories to update them.")
        exit()
